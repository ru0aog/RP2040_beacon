/**  
 * ============================================================================  
 *  LCD.cpp — Драйвер символьного дисплея HD44780 через PCF8574 (софтовый I2C)  
 * ============================================================================  
 *  
 *  НАЗНАЧЕНИЕ  
 *  ----------  
 *  Выводит служебную информацию маяка (позывной, режим, передаваемый текст) на  
 *  символьный ЖК-дисплей 1602/2004 с контроллером HD44780, подключённый через  
 *  I2C-расширитель портов PCF8574 (адрес LCD_ADDRESS = 0x27).  
 *  
 *  ПРОГРАММНЫЙ (BIT-BANG) I2C БЕЗ БИБЛИОТЕК  
 *  ----------------------------------------  
 *  Шина I2C реализована вручную на произвольных пинах SDA/SCL, задаваемых  
 *  динамически из массива device_DL[2]/[3] (soft_i2c_start / soft_i2c_stop /  
 *  soft_i2c_write / soft_i2c_read). Линии дёргаются в стиле open-drain через  
 *  смену режима пина: HIGH = INPUT_PULLUP (отпускаем линию, слабая внутренняя  
 *  подтяжка RP2040), LOW = OUTPUT+LOW (активно тянем вниз) — так исключаются  
 *  короткие замыкания при работе с +5 В модулем без внешних резисторов.  
 *  9-й такт (ACK) всегда отпускает SDA во вход, чтобы процессор не завис на  
 *  встречном ответе PCF8574. i2c_delay() = 10 мкс задаёт скорость шины.  
 *  
 *  ПРОТОКОЛ HD44780 (4-битный режим)  
 *  ---------------------------------  
 *  Байт передаётся двумя полубайтами (lcd_send -> lcd_write_nibble): данные +  
 *  биты управления (PIN_RS — команда/данные, PIN_RW — чтение/запись,  
 *  PIN_EN — строб, BACKLIGHT — подсветка) выставляются на P4-P7 расширителя,  
 *  затем контроллер фиксирует полубайт по спаду строба EN. Пошаговый сброс в  
 *  4-битный режим — в LCD_init (три посылки 0x30, затем 0x20).  
 *  Поддерживается и ЧТЕНИЕ статуса дисплея (lcd_read_nibble / lcd_read_status)  
 *  через переключение PIN_RW и приём полубайтов с P4-P7.  
 *  
 *  ВЫВОД ТЕКСТА И БЕГУЩАЯ СТРОКА  
 *  -----------------------------  
 *  lcd_set_cursor использует таблицу смещений DDRAM для 4 строк  
 *  (0x00/0x40/0x14/0x54). LCD_print печатает строку в заданную позицию.  
 *  LCD_push_char реализует каскадную бегущую строку: символы вдвигаются справа  
 *  в нижнюю строку (буфер lcd_scroll_buffers), а «уехавшие» влево переносятся  
 *  в верхнюю строку — эффект двухстрочной прокрутки; управляющие символы (<0x20)  
 *  игнорируются, экран после каждого сдвига «освежается» LCD_init(false).  
 *  
 *  API:  
 *    LCD_init(deep_init)           — инициализация дисплея (deep_init=true —  
 *                                    полная очистка DDRAM командой 0x01).  
 *    LCD_print(text, row, col)     — вывод строки (C-строка или String).  
 *    LCD_push_char(ch/str, row, w) — бегущая строка (символ или строка).  
 * ============================================================================  
 */
