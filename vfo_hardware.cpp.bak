 ...
 *  внешнего синтезатора Si5351. Сигнал-меандр выводится строго на GPIO 28  
 *  (физ. пин 34) и служит аварийным ВЧ-трактом для модуляторов IFKP / RTTY / CW,  
 *  когда чип Si5351 отсутствует или не отвечает.  
 *  
 *  ПРИНЦИП ГЕНЕРАЦИИ  
 *  -----------------  
 *  1. Тактовая ядра фиксируется на 120 МГц (или 133 МГц при VFO_CLOCK_133_MHZ),  
 *     реальное значение считается из калиброванного кварца VFO_CALIBRATED_XOSC_HZ.  
 *  2. Блок PIO гоняет микропрограмму из 2 инструкций (set 1 -> set 0), формируя  
 *     меандр: f_out = clk_sys / (2 * D), где D — делитель clkdiv в формате  
 *     fixed-point 16.8 (16 бит целая часть + 8 бит дробная).  
 *  3. Дробного 8-битного делителя недостаточно для точной частоты (на КВ его квант  
 *     ~13 кГц), поэтому дробная часть подстраивается на лету дельта-сигма  
 *     модуляцией делителя (vfo_dither_step).  
 *  
 *  МЕХАНИЗМЫ СНИЖЕНИЯ СПУРОВ (переключаются через #define в vfo_hardware.h)  
 *  -----------------------------------------------------------------------  
 *  VFO_USE_MASH2        — дельта-сигма 2-го порядка (MASH-1-1): переносит квантовый  
 *                         шум вверх по спектру и разбивает idle-тоны. Если выключено —  
 *                         работает формирователь 1-го порядка.  
 *  VFO_DITHER_RANDOMIZE — инжекция симметричного псевдослучайного дизера (Xorshift32,  
 *                         диапазон -7..+7 при VFO_DITHER_RAND_BITS=4, среднее строго 0) —  
 *                         превращает дискретные спуры в шумовой пол.  
 *  VFO_SNAP_TO_GRID     — привязка частоты в окне +-0.1 Гц к точке с минимальным  
 *                         dds_step (перебор +-10 сЦг). Эффективна на НЧ; на КВ окно  
 *                         слишком узкое и выигрыш мал.  
 *  
 *  РЕЖИМЫ ИСПОЛНЕНИЯ ДИЗЕРИНГА  
 *  ---------------------------  
 *  VFO_DITHER_ON_CORE1  — дизеринг выполняется в плотном бесконечном цикле на Core 1  
 *                         (vfo_core1_entry), обновляя clkdiv с частотой в единицы МГц.  
 *                         Параметры тона публикуются с Core 0 под аппаратным спинлоком  
 *                         (vfo_spin_lock) через флаг tone_changed; аккумуляторы фазы  
 *                         сбрасываются только на Core 1.  
 *  (иначе)              — дизеринг работает на Core 0 по таймерному прерыванию с  
 *                         интервалом VFO_DITHER_INTERVAL_US (напр. 10 мкс = 100 кГц).  
 *  
 *  ТОЧНОСТЬ / ОГРАНИЧЕНИЯ  
 *  ----------------------  
 *  Все внутренние расчёты частоты ведутся в сантигерцах (0.01 Гц), а dds_step  
 *  вычисляется двухэтапным делением со сдвигами (calculate_raw_params_chz), чтобы  
 *  избежать переполнения uint64 на КВ-частотах. Рабочий диапазон: 100 кГц ... 30 МГц.  
 *  vfo_hardware_init защищён от повторных вызовов (модемы переинициализируют VFO на  
 *  каждую передачу): PIO-программа грузится один раз (pio_program_loaded), Core 1  
 *  гасится через multicore_reset_core1() перед реконфигом PLL/PIO, а старый таймер  
 *  снимается (timer_already_running / cancel_repeating_timer).  
 *  ВАЖНО: любой out-of-band спур и гармоники меандра (3-я, 5-я, ...) в итоге  
 *  подавляет только аналоговый выходной ФНЧ на GPIO 28 — цифровые меры лишь  
 *  отодвигают шум туда, где фильтр эффективен.  
 *  
 *  ПУБЛИЧНЫЙ API  
 *  -------------  
 *  vfo_hardware_init(base_freq_hz, step_hz) — фиксация клока, настройка PIO,  
 *                         предрасчёт сетки из 33 тонов в ОЗУ, запуск дизеринга.  
 *  vfo_set_tone_instant(tone_index) — мгновенное переключение несущей на  
 *                         предрассчитанный тон (потокобезопасно для обоих режимов).  
 *  vfo_set_cw_key(key_down)        — телеграфная манипуляция (вкл/выкл выхода).  
 *  
 *  Все критичные ко времени функции помечены __not_in_flash_func и выполняются из  
 *  ОЗУ; в горячем пути дизеринга — только целочисленная арифметика.  
 * ============================================================================  
 */
...
#include "pico/multicore.h" // Поддержка многоядерности SDK
...
// Статические трекеры состояния для предотвращения утечек памяти при повторных init
...
/**
 * Расчет аппаратных коэффициентов частоты с Grid Snapping.
 */
static VfoParameters calculate_freq_params(unsigned int target_frequency_hz) {
    ...
    unsigned int p1 = 6, p2 = 2, fbdiv_int = 133; 
#else
...
void vfo_hardware_init(unsigned int base_freq_hz, double step_hz) {
    ...
    // Безопасно тушим Core 1 ПЕРЕД тем, как трогать PLL и PIO
    multicore_reset_core1();
#else
    // Если работаем по таймеру — отключаем прерывание прошлого сеанса
    ...
    // 2. Настройка системной тактовой частоты (120 МГц или 133 МГц)
...
#ifdef VFO_CLOCK_133_MHZ
    pll_init(pll_sys, 1, 1596000000ULL, 6, 2); 
    clock_configure(clk_sys, CLOCKS_CLK_SYS_CTRL_SRC_VALUE_CLKSRC_CLK_SYS_AUX, CLOCKS_CLK_SYS_CTRL_AUXSRC_VALUE_CLKSRC_PLL_SYS, 133000000, 133000000);
#else
    ...
    // 4. Предрасчет всей сетки частот в ОЗУ
