/**  
 * ============================================================================  
 *  si5351_driver.cpp — Драйвер синтезатора частот Si5351 (I2C, без библиотек)  
 * ============================================================================  
 *  
 *  НАЗНАЧЕНИЕ  
 *  ----------  
 *  Управляет внешним ВЧ-синтезатором Si5351 напрямую по I2C — формирует несущую  
 *  на выходах CLK0/CLK1 для модуляторов IFKP / RTTY / CW. Если чип на шине не  
 *  найден (SI_FAIL / device_SI[0]==0), команды управления ВЧ-выходом  
 *  прозрачно перенаправляются на программный VFO RP2040 (vfo_hardware).  
 *  
 *  ШИНА I2C  
 *  --------  
 *  Адрес чипа — SI5351_I2C_ADDR (0x60). Интерфейс выбирается динамически:  
 *  Wire или Wire1 по device_SI[1], пины SDA/SCL — из device_SI[2]/[3], частота  
 *  400 кГц. I2C_SI_restart() переинициализирует шину перед каждой транзакцией;  
 *  si5351_write_reg() делает до 3 попыток записи с перезапуском шины при сбое.  
 *  
