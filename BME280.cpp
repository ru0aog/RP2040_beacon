/**  
 * ============================================================================  
 *  BME280.cpp — Драйвер климатических датчиков BME280 / BMP280 / BMP180 (I2C)  
 * ============================================================================  
 *  
 *  НАЗНАЧЕНИЕ  
 *  ----------  
 ...
 *  
 *  ТРИ ТИПА ДАТЧИКОВ (автоопределение в initBME280 по Chip ID)  
 *  ----------------------------------------------------------  
 *    - BME280 (ID 0x60): температура + давление + ВЛАЖНОСТЬ;  
 *    - BMP280 (ID 0x58/0x56/0x57): температура + давление (без влажности);  
 *    - BMP180 (ID 0x55): старый чип, температура + давление, своя математика.  
 *  Тип хранится в enum SensorType detectedSensor; при неизвестном ID init  
 *  возвращает false и device_BM[0] сбрасывается в 0.  
 *  
 *  ШИНА I2C  
 *  --------  
 *  Адреса: BME280/BMP280 — 0x76 (BME280_ADDRESS), BMP180 — 0x77 (BMP180_ADDRESS).  
 *  Интерфейс выбирается динамически: Wire или Wire1 по device_BM[1], пины  
 *  SDA/SCL — из device_BM[2]/[3], частота 400 кГц. I2C_BME_restart()  
 *  переинициализирует шину перед обращениями к датчику.  
 *  
